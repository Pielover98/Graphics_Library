...
void main()
{
	...
	glm::mat4 light_view1 = glm::lookAt(-light_dir, glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));
	...
	while (context.step())
	{
		...
		loc = slot = 0;
		setUniforms(shdr_direct, loc, slot, cam_proj, cam_view, cube_model, light_proj1, light_view1, fb_shadow1.depthTarget);
		...
		loc = slot = 0;
		setUniforms(shdr_direct, loc, slot, cam_proj, cam_view, floor_model, light_proj1, light_view1, fb_shadow1.depthTarget);
		s0_draw(screen, shdr_direct, floor_geo);

		loc = slot = 0;
		setUniforms(shdr_direct, loc, slot, cam_proj, cam_view, ss_model, light_proj1, light_view1, fb_shadow1.depthTarget);
		s0_draw(screen, shdr_direct, ss_geo);
